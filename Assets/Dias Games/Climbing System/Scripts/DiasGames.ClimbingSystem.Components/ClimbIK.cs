...
namespace DiasGames.ClimbingSystem.Components
{
    public class ClimbIK : MonoBehaviour
    {
        ...
        [SerializeField] private LayerMask _footMask;
        [SerializeField] private AbilityTag _climbingIKTag;
        ...
        private RaycastHit CurrentHorizontalHit => _climbingComponent.CurrentCastData.ForwardHit;
        ...
        private bool _isRunningClimbing = false;
        private readonly int RightHandParamId = Animator.StringToHash("RightHand");
        private readonly int LeftHandParamId = Animator.StringToHash("LeftHand");
        private readonly int HandHorOffsetParamId = Animator.StringToHash("HandHorOffset");
        private readonly int FootIKParamId = Animator.StringToHash("FootIK");
        private readonly int HangWeightParamId = Animator.StringToHash("HangWeight");
        ...
        {
            _abilitySystem.OnAbilityStarted -= HandleAbilityStarted;
            _abilitySystem.OnAbilityStarted += HandleAbilityStarted;
            
            _abilitySystem.OnAbilityStopped -= HandleAbilityStopped;
            _abilitySystem.OnAbilityStopped += HandleAbilityStopped;
        }

        private void Update()
        {
            if (_isRunningClimbing)
            {
                RunIK();
            }
            else
        ...
        {
            if (layerIndex != 0 || !applyIK || !_abilitySystem.GetActiveTags().HasTag(_climbingIKTag, true)) return;
        ...
        private void ProccessHandIKEffectorOnLedge()
        {
            ...
            float horOffset = 1 - _animator.GetFloat(HandHorOffsetParamId);
            _leftHandOnLedgeEffector.localPosition += new Vector3(handIKOffset.x * horOffset, 0, 0);
            _rightHandOnLedgeEffector.localPosition += new Vector3(-handIKOffset.x * horOffset, 0, 0);
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                foreach (var hit in Physics.SphereCastAll(startPoint, 0.1f, Vector3.down, 
                             1.5f, _climbMask, QueryTriggerInteraction.Collide))
                {
                    if (hit.collider == CurrentHorizontalHit.collider && 
                        !hit.collider.CompareTag("Ladder") &&
                        hit.normal.y > 0.75f)
                    {
                        rightTop = hit.point;
                        rightHandHit = true;
            ...
            float rhWeight = _animator.GetFloat(RightHandParamId);
            float lhWeight = _animator.GetFloat(LeftHandParamId);

            // fix forward position
            if (CurrentHorizontalHit.collider != null)
            {
                ...
                Vector3 leftForward = transform.InverseTransformPoint(CurrentHorizontalHit.point);
                Vector3 rightForward = transform.InverseTransformPoint(CurrentHorizontalHit.point);
        ...
        private void ProccessFootIK()
        {
            ...
            float footWeight =  _animator.GetFloat(FootIKParamId);
            float hangWeight = _animator.GetFloat(HangWeightParamId);
        ...
        {
            if (ability.ActivationTags.HasTag(_climbingIKTag, true))
            {
                _isRunningClimbing = true;
            }
        }
        ...
        private void HandleAbilityStopped(Ability ability)
        {
            if (ability.ActivationTags.HasTag(_climbingIKTag, true))
            {
                if (!_abilitySystem.GetActiveTags().HasTag(_climbingIKTag, true))
                {
                    _isRunningClimbing = false;
                }
            }
        }
    }
